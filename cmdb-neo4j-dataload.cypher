...
// Load Nodes
// =============
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/pranay-graph/CMDB_GRAPH/refs/heads/main/nodes_Location.csv' AS row
MERGE (n:Location {instance_id: row.instance_id})
SET n.name = row.name, n.type = row.type, n.city = row.city, n.country = row.country;

LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/pranay-graph/CMDB_GRAPH/refs/heads/main/nodes_Rack.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/pranay-graph/CMDB_GRAPH/refs/heads/main/nodes_ComputerSystem.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/pranay-graph/CMDB_GRAPH/refs/heads/main/nodes_NetworkDevice.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/pranay-graph/CMDB_GRAPH/refs/heads/main/nodes_Storage.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/pranay-graph/CMDB_GRAPH/refs/heads/main/nodes_Subnet.csv' AS row
MERGE (n:Subnet {instance_id: row.instance_id})
SET n.cidr = row.cidr, n.vlan = row.vlan, n.purpose = row.purpose;

LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/pranay-graph/CMDB_GRAPH/refs/heads/main/nodes_IPAddress.csv' AS row
MERGE (n:IPAddress {instance_id: row.instance_id})
SET n.address = row.address, n.hostname = row.hostname;

LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/pranay-graph/CMDB_GRAPH/refs/heads/main/nodes_Application.csv' AS row
MERGE (n:Application {instance_id: row.instance_id})
SET n.name = row.name, n.tier = row.tier, n.env = row.env, n.owner = row.owner;

LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/pranay-graph/CMDB_GRAPH/refs/heads/main/nodes_Database.csv' AS row
MERGE (n:Database {instance_id: row.instance_id})
SET n.name = row.name, n.engine = row.engine, n.version = row.version, n.env = row.env;

LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/pranay-graph/CMDB_GRAPH/refs/heads/main/nodes_Cluster.csv' AS row
MERGE (n:Cluster {instance_id: row.instance_id})
SET n.name = row.name, n.cl_type = row.cl_type, n.env = row.env;

LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/pranay-graph/CMDB_GRAPH/refs/heads/main/nodes_Document.csv' AS row
MERGE (n:Document {instance_id: row.instance_id})
SET n.name = row.name, n.doctype = row.doctype, n.url = row.url;

LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/pranay-graph/CMDB_GRAPH/refs/heads/main/nodes_Incident.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/pranay-graph/CMDB_GRAPH/refs/heads/main/nodes_ChangeRequest.csv' AS row
